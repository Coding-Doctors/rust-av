...
impl EventHandler for Handler {
    ...
    fn on_guild_ban_addition(&self, _: Context, guild_id: GuildId, user: User) {
        ...
        if let Some(g) = cache.guild(guild_id) {
            ...
            let log_chan = format!("#{}", CONFIG.lock().log_channel);
                            ...
                            match ban.reason {
                                Some(r) => {
                                    ...
                                    if let Err(err) = id.say(&message) {
                                        println!(
                                            "Error sending message to channel {} for reason {}",
                                            id,
                                            err
                                        );
                                    }
                                ...
                                None => {
                                    let user_discrim =
                                        format!("{}{}", user.name, user.discriminator);
                                    let message = format!(
                                        "User {} was banned. No reason given",
                                        user_discrim
                                    );
                    ...
                    for member in guild.members.values() {
                        ...
                        if member.user.read().unwrap().id == owner_id {
                            ...
                            let info_string = format!(
                                "This server does not have a log channel as configured, please make sure that the channel exists and this bot has permission to access it"
                            );

...
pub fn ban_handler(_: Context, guild_id: GuildId, user: User, cfg: Config) -> Result<String, String> {
    ...
    let cached_guild = cache.guild(guild_id).ok_or_else(|| error!("No guild found for guild id {}", guild_id));
    let cache_unwrap = cached_guild.unwrap();
    let guild = cache_unwrap.read().unwrap();
